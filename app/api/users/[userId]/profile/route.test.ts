...
describe('User Profile API', () => {
    ...
    describe('GET /api/users/[userId]/profile', () => {
        it('should return 400 if userId is missing', async () => {
            ...
            expect(NextResponse.json).toHaveBeenCalledWith({ error: 'User ID is required.' }, { status: 400 });
            expect(response.status).toBe(400);
        });

        it('should return 404 if user profile not found in Supabase', async () => {
            mockSupabaseFrom.single.mockResolvedValue({ data: null, error: { message: 'Not found', code: 'PGRST116' } });
            ...
            expect(NextResponse.json).toHaveBeenCalledWith({ error: 'User profile not found.' }, { status: 404 });
        ...
        it('should handle unexpected errors', async () => {
            ...
            expect(NextResponse.json).toHaveBeenCalledWith({ error: 'An unexpected error occurred.' }, { status: 500 });
